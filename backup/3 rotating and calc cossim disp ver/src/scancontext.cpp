...
double scancontext::distance(const Descriptor& sc1, const Descriptor& sc2)
{
    ...
    // cossim 
    // auto cossim = [](std::vector<double> a, std::vector<double> b) -> double
    // {
    //     return std::inner_product(a.begin(), a.end(), b.begin(), 0); // last arg is init accumulation val.
    // };
    ...
    for (int shift_idx = 0; shift_idx < num_sectors; ++shift_idx)
    {
        ...
        std::transform( sc1_shift.begin(), sc1_shift.end(), sc2.begin(), 
                        ...
                        cossim // two arguments function 
        );
        printVector(sectors_cossim);
